# Brownfield Agent — proposalflow

**Run ID:** n977nkya6ybagzm3q6m5ycsny183r9ct
**Project ID:** jd76dd7qgw62g3wyxpck0shvcs83s8z8
**Repo:** /Users/damonbodine/Factory512/builds/proposalflow-20260325-202649/proposalflow
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Proposal Draft Generator — on proposal creation page, from contact profile + meeting notes + selected services + pricing, generate complete proposal: Executive Summary, Understanding of Needs, Proposed Solution, Timeline, Investment, Terms. Populate all text fields. 2) Meeting Notes Action Items Extractor — paste raw meeting notes, AI extracts: key discussion points, decisions made, action items with assignees/deadlines, follow-up topics. Structured output saved as meeting record. 3) Win/Loss Analysis — when proposal marked Won/Lost, analyze the proposal, client profile, and competition context. Generate insights on why it won or lost with recommendations. 4) Follow-Up Email Drafter — based on proposal status, last contact date, relationship stage, draft context-appropriate follow-up email. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/proposalflow-20260325-202649/proposalflow. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n977nkya6ybagzm3q6m5ycsny183r9ct"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n977nkya6ybagzm3q6m5ycsny183r9ct"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n977nkya6ybagzm3q6m5ycsny183r9ct","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n977nkya6ybagzm3q6m5ycsny183r9ct","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n977nkya6ybagzm3q6m5ycsny183r9ct","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/proposalflow-20260325-202649/proposalflow`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Proposal Draft Generator — on proposal creation page, from contact profile + meeting notes + selected services + pricing, generate complete proposal: Executive Summary, Understanding of Needs, Proposed Solution, Timeline, Investment, Terms. Populate all text fields. 2) Meeting Notes Action Items Extractor — paste raw meeting notes, AI extracts: key discussion points, decisions made, action items with assignees/deadlines, follow-up topics. Structured output saved as meeting record. 3) Win/Loss Analysis — when proposal marked Won/Lost, analyze the proposal, client profile, and competition context. Generate insights on why it won or lost with recommendations. 4) Follow-Up Email Drafter — based on proposal status, last contact date, relationship stage, draft context-appropriate follow-up email. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n977nkya6ybagzm3q6m5ycsny183r9ct",
  "projectName": "proposalflow",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Proposal Draft Generator — on proposal creation page, from contact profile + meeting notes + selected services + pricing, generate complete proposal: Executive Summary, Understanding of Needs, Proposed Solution, Timeline, Investment, Terms. Populate all text fields. 2) Meeting Notes Action Items Extractor — paste raw meeting notes, AI extracts: key discussion points, decisions made, action items with assignees/deadlines, follow-up topics. Structured output saved as meeting record. 3) Win/Loss Analysis — when proposal marked Won/Lost, analyze the proposal, client profile, and competition context. Generate insights on why it won or lost with recommendations. 4) Follow-Up Email Drafter — based on proposal status, last contact date, relationship stage, draft context-appropriate follow-up email. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/proposalflow-20260325-202649/proposalflow`
